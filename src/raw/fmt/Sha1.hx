...
@:dce private abstract Sha1Context(Ptr) to Ptr {
	...
	public inline function new():Void {
		this = Raw.malloc(CAPACITY, false);
		x80[0] = 0x80;
		x0[0] = 0;
	}
	public inline function reset():Void {
		Raw.memset(this, 0, CAPACITY - 2);
	}
}
...
class Sha1{
	...
	static function update(ctx: Sha1Context/*Context*/, input: Ptr/*Buffer*/, ilen:Int /*BufferSize*/):Void {
		...
		if ( (j + ilen) > 63) {
			...
			Raw.memcpy((ctx.Buffer:Ptr) + j, input, i);
		...
		ilen -= i;
		if (ilen == 1)
			ctx.Buffer[j] = input[i];
		else if (ilen > 0)
			Raw.memcpy((ctx.Buffer:Ptr) + j, input + i, ilen);
	}
