...
import raw.Ptr;
import raw.Ptr.Memory.setI32;
import raw.Ptr.Memory.getI32;
import raw.Ptr.Memory.getByte;
import raw.Ptr.Memory.setByte;
import raw._macros.AES128Macros.*;
...
Ported by r32
*/
#if !macro @:build(raw.Struct.make()) #end
...
class AES128 {
	...
	/**
	 ...
	 var input = Raw.mallocFromBytes(file, 16);
	 var output = input;
	...
	*/
	static public function cbcEncryptBuff(input: Ptr, key: Ptr, output: Ptr, length:Int, iv:Ptr/*16 bytes*/):Void {
		...
		if (remainders > 0) {
			if (input != output) BlockCopy(output, input);
			Raw.memset(output + remainders, 0, KEYLEN - remainders); /* add 0-padding */
			Cipher(output);
		}
	}
	...
	static function cbcDecryptBuffIO(io: Ptr, key:Ptr, length:Int, iv:Ptr): Void {
		...
		if (remainders > 0) {
			Raw.memset(io + remainders, 0, KEYLEN - remainders); // add 0-padding
