...
@:dce private abstract Md5Context(Ptr) to Ptr {
	...
	public inline function new() {
		this = alloc(CAPACITY, true);
		padding[0] = 0x80;
	}

	public inline function reset():Void {
		Mem.memset(this, 0, CAPACITY - 64);
	}
}
...
class Md5 {
	...
	static function update(ctx: Md5Context, input: Ptr, ilen:Int):Void {
		...
		if (left > 0 && ilen >= fill) {
			Mem.memcpy((ctx.buffer:Ptr) + left, input, fill);
		...
		if (ilen > 0) Mem.memcpy((ctx.buffer:Ptr) + left, input, ilen);
	}
	...
	static function finish(ctx: Md5Context, output: Ptr):Void {
		...
		Memory.setI32(cast ctx.msglen, low);
		Memory.setI32((cast ctx.msglen:Int) + 4, high);
		...
		Mem.memcpy(output, ctx.state, 16);
	}
}