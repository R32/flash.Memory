...
class Sha1{
	...
	static function update(ctx: Sha1Context/*Context*/, input: Ptr/*Buffer*/, ilen:Int /*BufferSize*/):Void {
		...
		if ( (j + ilen) > 63) {
			...
			Mem.memcpy((ctx.Buffer:Ptr) + j, input, i);
		...
		if (ilen == 1)
			...
			Mem.memcpy((ctx.Buffer:Ptr) + j, input + i, ilen);
	}
